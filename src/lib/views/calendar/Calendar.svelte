<script lang="ts">
	...
	import EventTitle from '$lib/components/calendar/EventTitle.svelte';
	...
	import { TaskEventGroup, diff } from '$lib/components/calendar/data';
	...
	onMount(() => {
		...
		const eventGroup: TaskEventGroup = new TaskEventGroup(db, rootId, (updatedEventIds) => {
	...
	let option: CalendarOptions = {
		...
		eventChange(arg) {
			...
			db.doc.transact((transaction) => {
				...
				if (event.start != fEvent.start?.valueOf()) {
					if (!fEvent.start) {
						throw new Error(`invalid fEvent.start, fEvent.start:${fEvent.end}, id:${fEvent.id}`);
					}
		...
		eventReceive(arg) {
			...
			let end: number = 0;
			if (fEvent.allDay) {
				const defaultDuration = calendar.getOption('defaultAllDayEventDuration') as number;
				end = start + defaultDuration;
			} else {
				const defaultDuration = calendar.getOption('defaultTimedEventDuration') as number;
				end = start + defaultDuration;
			}
...
<div class="flex h-full rounded bg-white p-4 shadow-xl" style="overflow: hidden;">
	<ScrollArea
		class="flex h-full w-full whitespace-nowrap rounded-md border"
