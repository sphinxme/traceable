<script lang="ts">
    ...
    import CollapseIcon from "./CollapseButton.svelte";
    import { fade, scale, slide } from "svelte/transition";
    import { cubicOut, linear, quadInOut, quartInOut } from "svelte/easing";
    import { onMount } from "svelte";
    import { Draggable } from "@fullcalendar/interaction/index.js";
    import CheckButton from "./CheckButton.svelte";
    import { EllipsisVertical } from "lucide-svelte";
    import ItemMenuButton from "./ItemMenuButton.svelte";
...
<div data-task-id={taskId} bind:this={containerEl} class="flex flex-col">
    <div class="todo">
        <TodoItem
            bind:this={todoItem}
        ...
        >
            <Overlay slot="overlay">
                <div
                    class="menu-btn opacity-0 transition-opacity duration-300 ease-out flex flex-row items-center"
                >
                    <ItemMenuButton
                        {taskId}
                        on:delete={() => {
    ...
    {#if children.length && !folded}
        <div class="flex flex-row" transition:slide>
            <div class="pb-4 w-9 pl-1 flex flex-row items-start">
                <div class=" bg-slate-300 h-full" style="width: 1px;"></div>
            </div>
